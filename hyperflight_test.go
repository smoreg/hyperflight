...
func TestForget(t *testing.T) {
	...
	secondResult := g.DoChan("key", func() (interface{}, error) {
		<-unblockSecond
		return 2, nil
	})
	...
	thirdResult := g.DoChan("key", func() (interface{}, error) {
		return 3, nil
	})
	...
	r := <-thirdResult
	if r.Val != 2 {
		t.Errorf("We should receive result produced by second call, expected: 2, got %v", r.Val)
	}
}
